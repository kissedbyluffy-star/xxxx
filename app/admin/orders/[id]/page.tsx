...
export default function AdminOrderDetailPage({ params }: { params: { id: string } }) {
  ...
  const fetchOrder = async () => {
    ...
    if (!token) return;
    const res = await fetch(`/api/admin/orders?order_id=${params.id}`, {
  ...
  if (!order) {
    return <div className="text-sm text-white/60">Loading order...</div>;
  }
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-2">
        <div className="rounded-xl border border-white/10 bg-white/5 p-4">
          ...
          <pre className="mt-2 whitespace-pre-wrap text-xs text-white/60">
            {JSON.stringify(order.payout_details, null, 2)}
          </pre>
        </div>
        <div className="rounded-xl border border-white/10 bg-white/5 p-4">
