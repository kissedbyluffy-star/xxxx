...
export default function OrderPage({ params }: { params: { publicId: string } }) {
  ...
  const fetchOrder = async () => {
    if (!token) return;
    setLoading(true);
    const res = await fetch(`/api/orders/${params.publicId}?t=${token}`);
    if (!res.ok) {
  ...
  const updatePayout = async () => {
    if (!token) return;
    setError(null);
    const res = await fetch(`/api/orders/${params.publicId}/payout?t=${token}`, {
      method: 'PATCH',
  ...
  const submitTxid = async () => {
    if (!token) return;
    setError(null);
    const res = await fetch(`/api/orders/${params.publicId}/txid?t=${token}`, {
      method: 'PATCH',
      headers: { 'Content-Type': 'application/json' },
