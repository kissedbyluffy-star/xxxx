    ...
    <div className="mt-6 space-y-4">
      {TIMELINE_STEPS.map((step, index) => {
        ...
        return (
          ...
          >
            <div className={`h-3 w-3 rounded-full ${isActive ? 'bg-emerald-400 shadow-glow' : 'bg-white/20'}`} />
            <div className="flex flex-col">
              <span className={`text-sm ${isActive ? 'text-white' : 'text-white/50'}`}>{STATUS_LABELS[step]}</span>
              {index === currentIndex && (
